...
namespace INventory_Project1.Controllers
{
    ...
    public class UnitController : Controller
    {
        ...
        public IActionResult Create(Unit unit)
        {
            ...
            try
            {
                if (unit.Description == null || unit.Description.Length < 4)
                {
                    errMessage = "Unit Description Must be at least 4 Characters";
                }

                if (_unitRepo.IsUnitNameExists(unit.Name))
                {
                    errMessage += " Unit Name " + unit.Name + " Already Exists";
                }
        ...
        public IActionResult Edit(int id)
        {
            Unit unit = _unitRepo.GetUnit(id);
            TempData.Keep();
            TempData["SuccessMessage"] = "Unit " + unit.Name + " Unit Saved Successfully";
